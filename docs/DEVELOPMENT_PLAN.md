...
QWidget 方法各自猜测状态。第四、五阶段已完成 Scene Clinic 完整视图：Rule Array、Spectrum、
...
截断原因。Root Cause Lens 已独立出宿主无关中文 Presenter、控制条、候选卡和横向证据带；报告会
拒绝陈旧焦点、外部候选、路径方向漂移与实测/结构候选不一致。生产 Atlas 进入 Lens 后按真实 DG
距离形成因果走廊，退出时恢复原布局；真实 Maya 2025 已用五节点绑定驱动链完成 1480px 与 800px
视觉验收。完整百万节点布局与视图虚拟化仍属于后续工作，不能用百万边查询基准替代渲染验收。
...
### 2026-08-27 阶段收口：Atlas 增量语义窗口

已完成纯 Python 窗口规划/差异算法、并行边稳定身份、Qt 图元增量复用、镜头保持、中文实时遥测、
百万边可复现基准与 1,200 节点 Maya 生成夹具。真实 Maya 2025 已证明捕获约 4.5 秒、语义换窗
约 4.2–5.2 毫秒；同时暴露整视图绘制仍约 500–580 毫秒。当前提交加入 120 / 480 大场景档位，
按展示版收口暂停，不宣称该档真实宿主绘制达标。下一轮首项是复测并定位 Maya 内嵌视图绘制成本，
而不是继续扩张功能面。

### 2026-08-27 发布恢复加固

静态隔离审计发现旧安装器的卸载可恢复，但“升级覆盖”没有回退点。现改为：相同安装幂等；受管
Module 变化时先原子保留 `pre-upgrade`；CLI `restore --backup` 只接受目标目录内的受管备份，
恢复时保留原备份，并为被替换的活动 Module 再建 `pre-restore`。Release ZIP 隔离回放不再直接
移动备份，而是走同一公开恢复入口。该闭环已用普通 Python 临时目录和解压发布包完成测试；本轮
遵守暂停真实 Maya 的要求，未新增 GUI 宿主声明。
