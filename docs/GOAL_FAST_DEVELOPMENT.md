...
## 第四十二里程碑阶段提交（Atlas 增量语义窗与百万边基准）

1. 新增宿主无关的语义窗口规划与精确 diff，节点、关系、Plug、并行边序号共同构成稳定身份。
2. 生产 Atlas 改为复用真实 Qt 节点/边图元，语义换窗保持镜头，并以中文覆盖层显示窗口、复用和耗时。
3. 普通场景预算 240 / 960；千节点以上自适应为 120 / 480，并关闭大场景逐节点阴影。
4. Maya 2025 离屏百万边基准：100,000 节点 / 1,000,000 边，首次应用 80.9 ms、首次绘制
   38.5 ms、折叠换窗 55.5 ms、换窗绘制 39.6 ms，契约全部通过。
5. 真实 Maya 2025 自生成场景捕获 1,254 节点 / 4,809 边，捕获约 4.5 s，语义换窗约
   4.2–5.2 ms；同时修复捕获成功后状态条未隐藏及逐节点 `cmds.lockNode` 导致的严重采集瓶颈。
6. 真实 Maya 内嵌整视图绘制仍测得约 500–580 ms；新 120 / 480 档位按本轮快速展示要求暂停
   复测，因此明确记录为下一轮验证项，不冒充通过。

本里程碑以可安装展示版阶段提交收口，`/goal` 保持 active；下一轮从真实宿主绘制预算复测继续。

## 第四十三里程碑进度（完成：可回退升级与显式 Module 恢复）

1. 安装相同 Module 变为真正幂等，不重复改写文件或制造备份。
2. 升级受管 Module 前原子保留 `pre-upgrade` 备份；写入失败不会先破坏当前活动 Module。
3. 新增 `install restore --backup`，只接受目标 Maya 2025 Module 目录内、命名正确且带 MayaScope
   受管标记的明确备份，不按时间猜测文件。
4. 恢复不消费原备份；若已有不同的受管 Module，先建立 `pre-restore` 回退点；第三方同名文件仍
   fail closed，绝不覆盖。
5. Release ZIP 隔离回放已改走公开恢复命令，不再由测试器私下移动文件；5 项部署/隔离回归通过。
6. 本轮没有启动真实 Maya，也没有把静态回放扩大声称为 GUI 已验证；最新 ZIP 的真实宿主复演仍
   沿用上一里程碑证据，待下一次允许宿主测试时重跑。
