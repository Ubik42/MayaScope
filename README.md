...
| --- | --- |
| 场景太大，不知道里面到底有什么 | 捕获一份不可变场景快照，把层级、连接、Reference、插件和外部依赖整理成可调查的 Scene Atlas |
| 错误很多，不知道是不是同一个事故 | Scene Clinic 运行规则并把共享节点、Reference、Namespace 或 DG 邻接的问题聚合成 Incident |
| 报错发生在 A，真正原因可能藏在上游 | Root Cause Lens 沿 DG 向上游找来源、向下游看影响，并展示节点路径、Plug 连接和排序依据 |
| Maya 很慢，但不知道是谁拖慢的 | 读取真实 Maya Profiler 事件；还可暂时旁路可疑节点，用 AB/BA 成对实验判断它是否真的造成退化 |
| 纹理、缓存和序列经常交漏 | 追踪 UDIM、帧序列、Alembic、USD、GPU Cache 等依赖，报告缺文件、已观测序列内部断档和工作区外路径 |
| 想自动修，又怕脚本把场景改坏 | 先生成可检查的 ChangePlan，再在单次 Undo 中执行，随后重新捕获并验证原问题是否真的消失 |
| 几十个镜头需要统一发布检查 | 在隐藏 Maya 中串行审计，保存签名报告；支持安全暂停、断点恢复、回归基线和项目级发布门禁 |
| 文件一打开就卡死或崩溃 | 开发中的 Crash Bisect 在安全副本上反复缩小 `.ma` 候选集，尝试隔离最小故障内容 |

它不是自动绑定器，也不是承诺“一键让所有 Maya 场景变快”的黑盒优化器。MayaScope 的职责是把
复杂场景变成**看得见的结构、说得清的证据和可以安全执行的操作计划**；最终业务判断仍由 TD 掌握。
...
收到一个可疑 Maya 文件
    ↓
捕获场景快照：建立节点、连接、引用、插件与文件依赖档案
    ↓
Scene Clinic 体检：把零散报错整理成可解释的问题和事故
    ↓
点击问题：Maya 选择、Scene Atlas、证据栏和根因透镜同步定位
    ↓
需要时进行 Profiler 采样或节点旁路对照实验
    ↓
预览 ChangePlan：确认影响对象、引用边界和可回滚条件
    ↓
单次 Undo 内执行 → 重新捕获 → 验证问题是否消失
    ↓
保存签名报告，并与已批准版本比较是否新增问题或性能退化
```
